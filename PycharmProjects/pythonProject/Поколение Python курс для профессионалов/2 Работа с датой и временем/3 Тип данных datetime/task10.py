...
# а значением — кортеж, первый элемент которого — время начала решения,
...
def interval_data(tup):
    first = datetime.strptime(tup[0], '%d.%m.%Y %H:%M:%S')
    second = datetime.strptime(tup[1], '%d.%m.%Y %H:%M:%S')

    return datetime.fromtimestamp(second.timestamp() - first.timestamp())
...
print(min(data, key=lambda x: interval_data(data[x])))