...
group_students = {name: list(group) for name, group in groupby(sorted(students, key=sort_func), key=sort_func)}
for i, k in group_students.items():
    print(i)
    print(k)

most_name = max(group_students, key=lambda name: len(group_students[name]))
print(most_name)