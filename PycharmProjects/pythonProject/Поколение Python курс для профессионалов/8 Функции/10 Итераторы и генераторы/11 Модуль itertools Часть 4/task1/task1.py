...
# Сортируем список по росту
sorted_persons = sorted(persons, key=lambda x: x.height)

# Группируем людей по росту
grouped_persons = {height: list(group) for height, group in groupby(sorted_persons, key=lambda x: x.height)}

print(grouped_persons[158])
# Выводим группы
for height, people in grouped_persons.items():
    names = ', '.join(sorted(person.name for person in people))
