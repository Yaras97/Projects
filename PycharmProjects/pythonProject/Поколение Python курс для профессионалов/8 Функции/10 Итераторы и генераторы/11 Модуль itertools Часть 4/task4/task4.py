...
sorted_func = lambda x: x[0]
result = {doit: list(map(lambda k: k[1:], sorted(other, key=lambda x: x[2])))  for doit, other in groupby(sorted(tasks, key=sorted_func), key=sorted_func)}
